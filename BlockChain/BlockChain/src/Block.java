...
public class Block {
  ...
  public Block (int num, int amount, Hash prevHash, long nonce) {
    ...
    this.nonce = nonce;
    // Based on the parameters, determine the hash of the block and set it.
    this.hash = new Hash(ByteBuffer.allocate(16 + (4 * this.prevHash.getData().length))
                                   .putInt(this.num)
                                   .putInt(this.amount)
                                   .put(this.prevHash.getData())
                                   .putLong(this.nonce)
                                   .array());
  ...
  public void mine() {
    ...
    do { // Iterate through all possible nonces, then set the new hash.
      this.hash = new Hash(ByteBuffer.allocate(16)
                                     .putInt(this.num)
                                     .putInt(this.amount)
                                     .putLong(this.nonce)
                                     .array());
  ...
  public String toString() {
    return String.format(
      ...
      this.num, this.amount, this.nonce, (num == 0) ? "null" : this.prevHash.toString(), this.hash.toString());
  } // toString()
