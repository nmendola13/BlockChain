...
public class BlockChain {
  ...
  public BlockChain(int initial) {
    ...
    Block firstBlock = new Block(0, initial, new Hash(ByteBuffer.allocate(4).putInt(0).array()));
    // Initialize the first and last pointers to the initial block.
    this.first = this.last = new Node(firstBlock);
  } // BlockChain(int)
  ...
  private int[] balances() {
    ...
    // Return Alexis' and Blake's balances
    return new int[] {this.first.data.getAmount() - total, total};
  } // balances()

  public static void main (String[] args) {
    BlockChain newChain = new BlockChain(100);
